...
from tab_cli.formats.sqlite import _quote_identifier
from tab_cli.url_parser import parse_url


def split_duckdb_url(url: str) -> tuple[str, str]:
    """Split a DuckDB input URL into database URL and table name."""
    db_url, separator, table_name = url.rpartition("#")
    if separator == "":
        raise ValueError(
            "DuckDB input must be specified as {url}#{table_name}, for example data.duckdb#users"
        )
    if len(db_url) == 0:
        raise ValueError("DuckDB input must include a database URL before '#'")
    if len(table_name) == 0:
        raise ValueError("DuckDB input must include a table name after '#'")
    return db_url, table_name


class DuckdbFormat(FormatHandler):
    """Handler for DuckDB database tables."""

    def extension(self) -> str:
        return "duckdb"

    def source_url(self, url: str) -> str:
        db_url, _ = split_duckdb_url(url)
        return db_url
    ...
    def scan(
        ...
        _, table_name = split_duckdb_url(url)
    ...
    def read_stream(self, stream: BinaryIO) -> pl.DataFrame:
        raise ValueError("DuckDB input does not support stdin; pass a database path as {url}#{table_name}")
    ...
    def write(self, lf: pl.LazyFrame) -> Iterable[bytes]:
        raise ValueError("DuckDB output is not supported")
    ...
    def _read_query(
        ...
        try:
            ...
            raise ImportError(
                "Package 'duckdb' is required for DuckDB input. "
                "Install with: pip install 'tab-cli[duckdb]'"
            ) from e
        ...
        if opener is None:
            raise ValueError("DuckDB remote input requires a storage backend opener")

        with opener(db_url) as stream:
            with NamedTemporaryFile(suffix=".duckdb", delete=False) as temp_file:
